...
void calibrate()
{
	...
	// Creating vector to store vectors of 2D points for each checkBoard image
	// 创建矢量以存储每个棋盘图像的二维点矢量
	...
	// Path of the folder containing checkBoard images
	// 包含棋盘图像的文件夹的路径
	...
	for (int i{ 0 }; i < images.size(); i++)
	{
		...
		success = cv::findChessboardCorners(gray, cv::Size(checkBoard[0], checkBoard[1]), corner_pts, CALIB_CB_ADAPTIVE_THRESH | CALIB_CB_FAST_CHECK | CALIB_CB_NORMALIZE_IMAGE);
	...
	*/
	...
	cv::FileStorage fs("/params.xml", cv::FileStorage::WRITE); // create FileStorage object
	fs << cameraMatrix << distCoeffs << R << T; // command to save the data
